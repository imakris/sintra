...
- Cirrus now limits the FreeBSD `ctest` invocation to the `sintra.basic_pubsub`, `sintra.barrier_stress`, and `sintra.processing_fence` suites so that logs highlight representative failures instead of repeating identical 120 s stalls across the full matrix.【F:.cirrus.yml†L21-L24】

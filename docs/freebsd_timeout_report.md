...
- A subsequent Cirrus attempt emitted no `sintra.trace` lines at all, confirming the harness lost the tracing environment. The FreeBSD job now exports `SINTRA_TRACE_SYNC=1` and the widened `SINTRA_TRACE_SCOPE` directly on the `ctest` command line so future runs always capture the bootstrap instrumentation even if CTest properties are skipped.【F:.cirrus.yml†L22-L24】
- Another run failed earlier with `ctest: not found` because the YAML list split the trace-export command across two entries. The job now invokes `env SINTRA_TRACE_SYNC=1 SINTRA_TRACE_SCOPE=… ctest …` in a single list item so the environment variables are applied without breaking the command path.【F:.cirrus.yml†L22-L24】

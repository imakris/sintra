...
- A fence entered from a request-reader thread also skips the stream of
  every other request-reader thread of the process that is parked in a
  fence at the same time. Such a thread has stopped reading its own ring,
  so waiting for it would mean waiting for a thread that is waiting back;
  otherwise two handler-invoked fences in one process would wait for each
  other forever. A fence entered concurrently from two request-reader
  threads therefore proves nothing about the streams those threads serve.
  A caller on a control thread serves no ring and cannot close such a
  cycle, so it keeps the full guarantee: if the proof is needed, call the
  barrier from a control thread.

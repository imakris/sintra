...
`shutdown` is the standard terminal API for the multi-process Sintra
lifecycle. All live participants enter the same collective shutdown
protocol; the runtime owns the rendezvous, optional coordinator hook,
drain wait, and teardown. The simple form is a symmetric collective
shutdown. The overload that takes
[`shutdown_options`](shutdown_options.md) lets the coordinator run a
bounded side effect inside the protocol.
...
- All swarm participants reach the same top-level handoff and are ready to
  tear down together.
- The coordinator must run a bounded side effect (write a summary file,
  flush metrics) at a defined point inside the shutdown protocol; pass a
  `shutdown_options` with a `coordinator_shutdown_hook`.
- A single-process program needs the public terminal API. With no
  coordinator id present, `shutdown()` falls through to `detail::finalize`
  internally without a collective barrier.
...
- A second `shutdown()` call while a teardown is already in progress is
  rejected: the runtime throws `std::logic_error` to flag the illegal
  composition.
- Ordinary callers must not pair `shutdown()` with extra final
  `_sintra_all_processes` user barriers, additional `detail::finalize`
  calls, or duplicated `shutdown` invocations.
- After successful return, the local Sintra runtime is torn down: facade
  APIs are no longer usable until a new `init` is performed.
...
- The collective barrier blocks until every live participant arrives.
  Participants that have already left via `leave()` or have exited
  abnormally are excluded from the membership snapshot.
- If the coordinator hook throws, the runtime still finalises the local
  process and rethrows the exception to the caller of `shutdown(options)`.

Failures:

- Throws `std::logic_error` when called while another lifecycle teardown
  is already in progress.
- Propagates exceptions thrown by `coordinator_shutdown_hook` to the
  coordinator's caller after finalising local state.

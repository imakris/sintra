...
| `ring_abi_mismatch_exception` | A direct ring control file or lifecycle anchor was created by an incompatible Sintra ABI. | Use matching builds, or remove stale ring files or the persistent lifecycle anchor only when no process can still use that ring. See [Direct rings](reference/rings.md). |
...
| A Windows process exits with `0xC0000005`, `0xC000001D`, `0xC000008E`, or `3` where it used to exit with `1` | Expected. A faulted process now exits with the status that names its cause instead of a generic failure code. | Test for a non-zero exit status rather than a particular value. See [announce_fatal_windows_exception](reference/announce_fatal_windows_exception.md). |
| `#error "SINTRA_HAS_SEH=1 requires MSVC, clang-cl, or clang with -fms-extensions..."` | `SINTRA_HAS_SEH=1` was defined for a GCC build. GCC does not implement `__try`/`__except`. | Drop the define for MinGW GCC, or switch to clang and add `-fms-extensions`. See [Windows build switches](../README.md#windows-build-switches). |
| `#error "SINTRA_HAS_SEH=1 requires a Windows target"` | The macro was defined globally in a cross-platform build. | Scope it, for example `$<$<PLATFORM_ID:Windows>:SINTRA_HAS_SEH=1>`. |
| `[sintra] crash path did not complete - forcing termination` on stderr, and a faulted process that took a few seconds to disappear | The host's unhandled-exception filter ran longer than the crash watchdog grace period, so Sintra ended the process itself. | Expected if the reporter is slow; raise `SINTRA_CRASH_WATCHDOG_GRACE_MS`. If unexpected, the host filter is blocking. |
| A fault on a Sintra thread raises no `crash` lifecycle event | The build has `SINTRA_HAS_SEH=0`, or a debugger is attached, in which case the fault is left to the debugger. | Enable the guard, or have the host filter call [`announce_fatal_windows_exception`](reference/announce_fatal_windows_exception.md). |

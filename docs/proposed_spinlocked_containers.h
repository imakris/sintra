...
struct spinlocked
{
    ...
    // Compound access: calls fn(container&) under lock, returns fn's result.
...
struct spinlocked_map: detail::spinlocked<map, Key, T>
{
    ...
    std::optional<T> get_value(const Key& key) const
    {
        locker l(this->m_sl);
        auto it = this->m_c.find(key);
        if (it == this->m_c.end()) return std::nullopt;
        return it->second;
    }
    ...
    bool with_value(const Key& key, Fn&& fn) const
    {
        locker l(this->m_sl);
        auto it = this->m_c.find(key);
        if (it == this->m_c.end()) return false;
        fn(it->second);
        return true;
    }
};

...
//
// Demonstrates dynamically joining and leaving the process swarm at runtime.
// Two players start a ping-pong rally. After a configurable number of volleys
// a third player is spawned and joins the rally. Once enough three-way volleys
// have been exchanged the extra player exits, only to be brought back later.
//
...
int player_process()
{
    ...
    {
        barrier("dynamic-swarm-example-finished", "_sintra_all_processes");
        finalize();
        {
...
int main(int argc, char* argv[])
{
    ...
    if (process_index() == 0) {
        ...
        struct Coordinator_state
        {
            ...
            int entries_remaining = 2; // number of times the third player should join
        } state;

        const uint64_t two_player_threshold = 12;
        const uint64_t three_player_threshold = 9;
        const uint64_t final_two_player_threshold = 8;
        ...
        activate_slot([&](const Ball& ball)
        {
            ...
            if (state.entries_remaining == 0 && !state.final_phase && state.volley_count >= final_two_player_threshold) {
                console() << "Demonstration complete. Stopping rally.\n";
                world() << Stop{};

...
class TestRunner:
    ...
    def _ensure_windows_debugging_tools_installed(self) -> Tuple[bool, str]:
        ...
        def record_attempt(source: str, success: bool, detail: str) -> bool:
            if success:
                attempt_logs.append(f"{source}: success")
                return True

            message = detail.strip()
            attempt_logs.append(f"{source}: {message or 'failed'}")
            return False
        ...
        def install_with_winget() -> Tuple[bool, str]:
            ...
            try:
                ...
                return False, f'failed to invoke winget: {exc}'
        ...
        def install_with_chocolatey() -> Tuple[bool, str]:
            choco_path = shutil.which('choco')
            if not choco_path:
                return False, 'Chocolatey not available on PATH'
            ...
            try:
                ...
                return False, f'failed to invoke Chocolatey (windbg install): {exc}'

            if result.returncode == 0:
                return True, ''

            details = result.stderr.strip() or result.stdout.strip()
            return False, f'Chocolatey exited with code {result.returncode}: {details}'

        installed = record_attempt('winget (Microsoft.WinDbg)', *install_with_winget())
        if not installed:
            installed = record_attempt('Chocolatey (windbg)', *install_with_chocolatey())

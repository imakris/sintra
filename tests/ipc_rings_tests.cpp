...
TEST_CASE(test_reader_eviction_does_not_underflow_octile_counter)
{
    ...
    try {
        ...
        while (std::chrono::steady_clock::now() < guard_deadline) {
            sintra::Ring<uint32_t, true>::Reader_state_pack state(slot.reader_state.load(std::memory_order_acquire));
            if (state.has_guard()) {
        ...
        sintra::Ring<uint32_t, true>::Reader_state_pack old_state(slot.reader_state.load(std::memory_order_relaxed));
        slot.reader_state.store(old_state.with_guard(1).with_status(sintra::Ring<uint32_t, true>::READER_STATE_ACTIVE).value,
...
TEST_CASE(test_slow_reader_eviction_restores_status)
{
    ...
    sintra::Ring<uint64_t, true>::Reader_state_pack initial_state(
        static_cast<uint8_t>(trailing_octile), 1, sintra::Ring<uint64_t, true>::READER_STATE_ACTIVE);
    slot.reader_state.store(initial_state.value, std::memory_order_relaxed);

    // Evict: clear guard and set status to evicted atomically
    sintra::Ring<uint64_t, true>::Reader_state_pack evicted_state(
        static_cast<uint8_t>(trailing_octile), 0, sintra::Ring<uint64_t, true>::READER_STATE_EVICTED);
    slot.reader_state.store(evicted_state.value, std::memory_order_relaxed);
    control.read_access.fetch_sub(guard_mask, std::memory_order_relaxed);

    reader.done_reading_new_data();

    sintra::Ring<uint64_t, true>::Reader_state_pack restored_state(slot.reader_state.load(std::memory_order_acquire));
...
TEST_CASE(test_streaming_reader_status_restored_after_eviction)
{
    ...
    // Set initial state atomically: no guard, active status
    sintra::Ring<uint32_t, true>::Reader_state_pack init_state(0, 0, sintra::Ring<uint32_t, true>::READER_STATE_ACTIVE);
    slot.reader_state.store(init_state.value, std::memory_order_relaxed);

    auto first_range = reader.wait_for_new_data();
    ASSERT_TRUE(first_range.end >= first_range.begin);
    reader.done_reading_new_data();

    // Atomically get octile and clear guard, set status to evicted
    sintra::Ring<uint32_t, true>::Reader_state_pack old_state(slot.reader_state.load(std::memory_order_acquire));
    const uint8_t guarded_octile = old_state.octile();
    const uint64_t guard_mask = uint64_t(1) << (8 * guarded_octile);

    sintra::Ring<uint32_t, true>::Reader_state_pack new_state = old_state.with_guard(0).with_status(
        sintra::Ring<uint32_t, true>::READER_STATE_EVICTED);
    uint16_t expected = old_state.value;
    ASSERT_TRUE(slot.reader_state.compare_exchange_strong(expected, new_state.value, std::memory_order_acq_rel));
    ...
    sintra::Ring<uint32_t, true>::Reader_state_pack final_state(slot.reader_state.load(std::memory_order_acquire));

...
TEST_CASE(test_reader_eviction_does_not_underflow_octile_counter)
{
    ...
    try {
        ...
        while (std::chrono::steady_clock::now() < guard_deadline) {
            uint8_t guard_token =
                slot.state.guard_token.load(std::memory_order_acquire);
        ...
        while (std::chrono::steady_clock::now() < eviction_deadline) {
            auto status = slot.state.status.load(std::memory_order_acquire);
        ...
        slot.state.store_status_trailing_and_guard(
            sintra::Ring<uint32_t, true>::READER_STATE_ACTIVE,
            guarded_octile,
...
TEST_CASE(test_slow_reader_eviction_restores_status)
{
    ...
    slot.state.store_status_trailing_and_guard(
        sintra::Ring<uint64_t, true>::READER_STATE_ACTIVE,
        static_cast<uint8_t>(trailing_octile),
    ...
    slot.state.guard_token.store(0, std::memory_order_release);
    slot.state.status.store(
        sintra::Ring<uint64_t, true>::READER_STATE_EVICTED, std::memory_order_release);
    control.read_access.fetch_sub(guard_mask, std::memory_order_release);

    reader.done_reading_new_data();

    auto restored_status = slot.state.status.load(std::memory_order_acquire);
...
TEST_CASE(test_streaming_reader_status_restored_after_eviction)
{
    ...
    slot.state.guard_token.store(0, std::memory_order_release);
    slot.state.status.store(
        sintra::Ring<uint32_t, true>::READER_STATE_ACTIVE, std::memory_order_release);

    auto first_range = reader.wait_for_new_data();
    ASSERT_TRUE(first_range.end >= first_range.begin);
    reader.done_reading_new_data();

    const uint8_t guarded_octile =
        slot.state.trailing_octile.load(std::memory_order_acquire);
    ...
    uint8_t guard_snapshot =
        slot.state.guard_token.exchange(0, std::memory_order_acq_rel);
    ...
    slot.state.status.store(
        sintra::Ring<uint32_t, true>::READER_STATE_EVICTED, std::memory_order_release);
    ...
    ASSERT_EQ(active_state, slot.state.status.load(std::memory_order_acquire));

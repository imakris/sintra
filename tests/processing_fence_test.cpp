...
static std::atomic<bool> g_worker_handler_ran{false};
...
void log_event(const std::string& channel, std::string_view message)
{
    if (std::getenv("SINTRA_TRACE_BARRIER") == nullptr) {
        return;
    }
    ...
    auto path = shared_directory() / (channel + ".log");
...
int controller_process()
{
    ...
    auto handler_done_deactivator = activate_slot([
        ...
        log_event("controller", "handler_done_signal");
        bool expected = false;
    ...
    log_event("controller", "dispatch_work");
    world() << Work_message{};

    log_event("controller", "processing_barrier_begin");
    const auto start = std::chrono::steady_clock::now();
    const bool barrier_result = barrier<processing_fence_t>(
        "processing-fence", group);
    log_event("controller", std::string("processing_barrier_end result=") +
        (barrier_result ? "success" : "failure"));
    ...
    out << (barrier_result ? "barrier_true" : "barrier_false") << '\n';

    log_event("controller", std::string("handler_done_future=") +
        (handler_done ? "ready" : "pending"));
...
int worker_process()
{
    ...
    auto slot = [](const Work_message&) {
        std::cout << "[worker] handler_start" << std::endl;
        log_event("worker", "handler_start");
        std::this_thread::sleep_for(kHandlerDelay);
        std::cout << "[worker] handler_before_done" << std::endl;
        log_event("worker", "handler_before_done");
        world() << Handler_done{};
        std::cout << "[worker] handler_after_done" << std::endl;
        log_event("worker", "handler_after_done");
        g_worker_handler_ran.store(true, std::memory_order_relaxed);
    };
    activate_slot(slot);
    ...
    log_event("worker", "after_processing_barrier");
    log_event("worker", g_worker_handler_ran.load(std::memory_order_relaxed) ? "handler_ran=true" : "handler_ran=false");
    barrier("processing-fence-test-done", "_sintra_all_processes");

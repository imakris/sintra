...
int player_process()
{
    ...
    sintra::console() << "[Test Player] ready\n";
...
int bench_process()
{
    ...
    sintra::activate_slot([&](const BenchSwingRequest& request) {
        ...
        sintra::console() << "[Test Bench] swing ack sent\n";
    });
    ...
    sintra::activate_slot([&](const BenchDepartRequest& request) {
        ...
        sintra::console() << "[Test Bench] depart acknowledged\n";
        std::lock_guard<std::mutex> lock(stop_mutex);
        should_stop = true;
        stop_cv.notify_all();
    });
    ...
    sintra::console() << "[Test Bench] ready with iid=" << self << "\n";
...
int main(int argc, char* argv[])
{
    ...
    bool bench_spawned = false;
    bool bench_exited = false;
    ...
    if (sintra::process_index() == 0) {
        ...
        sintra::activate_slot([&](const Hit& hit) {
...

            {
                std::lock_guard<std::mutex> lock(state_mutex);
                ++hit_count;
                sintra::console() << "[Test Controller] hit " << hit_count
            ...
            if (request_spawn) {
                ...
                if (spawned != sintra::invalid_instance_id) {
                    sintra::console() << "[Test Controller] spawned bench iid=" << spawned << "\n";
                    bench_spawned = true;
                    bench_id = spawned;
                }
                else {
                    sintra::console() << "[Test Controller] bench spawn failed, will retry\n";
                    bench_spawn_requested = false;
                }
            }

            if (request_depart && depart_target != sintra::invalid_instance_id) {
                sintra::console() << "[Test Controller] requesting bench depart iid=" << depart_target << "\n";
        ...
        sintra::activate_slot([&](const BenchReady& ready) {
            ...
            if (swing_target != sintra::invalid_instance_id) {
                sintra::console() << "[Test Controller] bench ready, requesting swing iid=" << swing_target << "\n";
        ...
        sintra::activate_slot([&](const BenchSwingAck& ack) {
            ...
            bool request_depart = false;
            {
                ...
                sintra::console() << "[Test Controller] bench swing ack from iid=" << ack.bench_iid << "\n";
                if (bench_id == sintra::invalid_instance_id) {
            ...
            if (request_depart && depart_target != sintra::invalid_instance_id) {
                sintra::console() << "[Test Controller] confirming depart for iid=" << depart_target << "\n";
                sintra::world() << BenchDepartRequest{depart_target};
                ...
                std::thread([&, depart_target] {
...

                    {
                        ...
                        bench_exited = true;
                        sintra::console() << "[Test Controller] bench departed iid=" << depart_target << "\n";
                        state_cv.notify_all();
                    }
                }).detach();
            }
        ...
        sintra::activate_slot([&](StopAll) {
            std::lock_guard<std::mutex> lock(state_mutex);
            stop_seen = true;
            sintra::console() << "[Test Controller] stop acknowledged\n";
            state_cv.notify_all();
        });
        ...
        sintra::barrier("dynamic-swarm-test-finished", "_sintra_all_processes");
        sintra::finalize();
    }

    return (bench_spawned && bench_exited) ? 0 : 1;
}

...
{
public:
    explicit Membership_fixture(bool include_external_group = true)
    {
...
int main()
{
    ...
    {
        Membership_fixture without_external(/*include_external_group=*/false);
        const auto extra_joiner = sintra::make_process_instance_id(5);
        if (!without_external.enroll(extra_joiner, true)) {
            std::cerr << "optional: enrollment should succeed without external group" << std::endl;
            return 1;
        }

        const auto& membership = without_external.membership_map();
        auto membership_it = membership.find(extra_joiner);
        if (membership_it == membership.end()) {
            std::cerr << "optional: membership map missing entry" << std::endl;
            return 1;
        }

        const auto& joined_groups = membership_it->second;
        if (joined_groups.size() != 1) {
            std::cerr << "optional: unexpected number of default groups" << std::endl;
            return 1;
        }
    }

...
#include <random>
  ...
  #if !defined(SINTRA_USE_OS_SYNC_WAIT_ON_ADDRESS)
    // os_sync_wait_on_address is opt-in because macOS runners exhibited severe
    // performance regressions compared to the mature named-semaphore backend.
    #define SINTRA_USE_OS_SYNC_WAIT_ON_ADDRESS 0
  #endif

  #if defined(__has_include)
    #if __has_include(<os/os_sync_wait_on_address.h>) && __has_include(<os/clock.h>) && SINTRA_USE_OS_SYNC_WAIT_ON_ADDRESS

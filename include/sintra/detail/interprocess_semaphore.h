...
namespace {
inline void bounded_swprintf(wchar_t* buf, size_t cch, const wchar_t* fmt, unsigned long long x) {
#if defined(_MSC_VER)
    _snwprintf_s(buf, cch, _TRUNCATE, fmt, x);
#else
    swprintf(buf, cch, fmt, x);
#endif
}
inline void bounded_swprintf2(wchar_t* buf, size_t cch, const wchar_t* fmt, const wchar_t* s) {
#if defined(_MSC_VER)
    _snwprintf_s(buf, cch, _TRUNCATE, fmt, s);
#else
    swprintf(buf, cch, fmt, s);
#endif
}
inline void backoff_yield() noexcept {
...
struct ips_backend_win_state {
    ...
    wchar_t  name[64] = {0};
    HANDLE   h = nullptr;
    unsigned owner_pid = 0;  // Track which process owns this handle
};
...
inline ips_win_handle_cache& ips_win_handles() {
...
static bool ips_win_copy_name(ips_backend& b, const wchar_t* s) noexcept {
    auto& st = W(b);
    if (!s || !s[0]) { st.name[0] = L'\0'; return true; }
    // Do not modify st.name unless it fits
    size_t len = 0;
    while (s[len] != L'\0') ++len;
    if (len >= 63) { errno = ENAMETOOLONG; return false; }
    for (size_t i=0; i<=len; ++i) st.name[i] = s[i];
    return true;
}
...
static HANDLE ips_win_local_handle(ips_backend& b) noexcept {
    auto& st = W(b);
    if (st.name[0] == L'\0') { errno = EINVAL; return nullptr; }
    ...
    if (it != cache.map.end() && it->second) return it->second;
...
inline void ips_backend::init_default(uint32_t initial) noexcept {
    ...
    st.initial = initial;
    st.max = 0x7fffffff;
    if (initial > st.max) { errno = EINVAL; InterlockedExchange(&st.init_flag, 2); return; }
...
inline void ips_backend::init_named(uint32_t initial, const wchar_t* name, uint32_t max_count) noexcept {
    auto& st = W(*this);
    st.initial = initial;
    st.max = max_count;
    if (initial > max_count) { errno = EINVAL; InterlockedExchange(&st.init_flag, 2); return; }

    if (name && name[0] != L'\0') {
        st.was_autogenerated = false;
        if (!ips_win_copy_name(*this, name)) { InterlockedExchange(&st.init_flag, 2); return; }
    } else {
        st.was_autogenerated = true;
...
inline bool ips_backend::try_wait() noexcept {
    ...
    if (!h) { errno = EINVAL; return false; }
...
inline bool ips_backend::try_wait_for(std::chrono::nanoseconds d) noexcept {
    ...
    DWORD rc = WaitForSingleObject(ips_win_local_handle(*this), ms);
    if (rc == WAIT_OBJECT_0) return true;
    if (rc == WAIT_TIMEOUT) { errno = ETIMEDOUT; return false; }
    errno = EINVAL;
    return false;
}
...
inline void ips_backend::post(uint32_t n) noexcept {
    ...
    ips_win_ensure_handle(*this);
    HANDLE h = ips_win_local_handle(*this);
    if (!h) { errno = EINVAL; return; }
    if (n > (uint32_t)LONG_MAX) { errno = EOVERFLOW; return; }
...
inline void ns_to_timespec(uint64_t ns, struct timespec& ts) {
...
inline int futex_wait_(int* addr, int val, const struct timespec* rel) {
    return (int)syscall(SYS_futex, addr, FUTEX_WAIT, val, rel, nullptr, 0);
}
inline int futex_wake_(int* addr, int n) {
    return (int)syscall(SYS_futex, addr, FUTEX_WAKE, n, nullptr, nullptr, 0);
}
#endif

// Unified POSIX helpers (no further #ifs elsewhere)
inline int posix_wait_equal_until(uint32_t* addr, uint32_t expected, uint64_t deadline) noexcept {
#if SINTRA_BACKEND_DARWIN
    const uint32_t flags = OS_SYNC_WAIT_ON_ADDRESS_SHARED;
    for (;;) {
        int rc = os_sync_wait_on_address_with_deadline((void*)addr, (uint64_t)expected, 4,
                                                       flags, OS_CLOCK_MONOTONIC, deadline);
        if (rc >= 0) return 0;
    ...
    for (;;) {
        ...
        int rc = futex_wait_((int*)addr, (int)expected, &ts);
...
inline void posix_wake_some(uint32_t* addr, int n) noexcept {
#if SINTRA_BACKEND_LINUX
    if (n < 0) n = 0;
    if (n > INT_MAX) n = INT_MAX;
    (void)futex_wake_((int*)addr, n);
...
inline void ips_backend::init_named(uint32_t initial, const wchar_t*, uint32_t max_count) noexcept {
...
inline bool ips_backend::try_wait_for(std::chrono::nanoseconds d) noexcept {
    ...
    for (;;) {
        ...
        if (posix_wait_equal_until(&P(*this).count, 0u, deadline) == -1) {
            ...
            if (errno == ENOTSUP) { struct timespec ts{0, 1'000'000}; nanosleep(&ts, nullptr); }
...
public:
    ...
    explicit interprocess_semaphore(unsigned int initial = 0) noexcept {
        impl_.init_default(static_cast<uint32_t>(initial));
    }

    interprocess_semaphore(unsigned int initial, const wchar_t* name, uint32_t max_count = 0x7fffffff) noexcept {
        impl_.init_named(static_cast<uint32_t>(initial), name, max_count);
    }
    ...
    void wait() noexcept {
        while (!impl_.wait()) { /* retry on error */ }
    }

    // API compatibility: [[nodiscard]] bool try_wait()
    ...
    [[nodiscard]] bool timed_wait(const std::chrono::time_point<Clock, Duration>& abs_time) noexcept {
        ...
        return impl_.try_wait_for(delta_ns);
    }

    // API compatibility: try_wait_for with duration
    [[nodiscard]] bool try_wait_for(std::chrono::nanoseconds rel_timeout) noexcept {
        return impl_.try_wait_for(rel_timeout);
    }

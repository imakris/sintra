...
private:
    ...
    bool wait_os_sync_with_timeout(int32_t expected, std::chrono::nanoseconds remaining, int32_t& observed)
    {
        ...
        while (true) {
            using count_type = std::chrono::nanoseconds::rep;
            ...
            throw std::system_error(errno, std::generic_category(), "__ulock_wait2");
        }
    }
    ...
    void wake_one_waiter()
    {
        while (true) {
            ...
            throw std::system_error(errno, std::generic_category(), "__ulock_wake");
        }
    }
#else
    sem_t m_sem{};

...
inline int retrying_fcntl_get(int fd, int cmd)
{
    int rv;
    do {
        rv = ::fcntl(fd, cmd);
...
inline int retrying_fcntl_set(int fd, int cmd, int value)
{
    int rv;
    do {
...
inline int system_pipe2(int pipefd[2], int flags)
{
    ...
    const auto set_flag = [&](int fd, int cmd, int value) {
        int current = retrying_fcntl_get(fd, cmd == F_SETFD ? F_GETFD : F_GETFL);
        if (current == -1) {
            return -1;
        }
        return retrying_fcntl_set(fd, cmd, current | value);
    };
...
bool spawn_detached(const char* prog, const char * const*argv, int* child_pid_out = nullptr)
{
    ...
    int pipe_rv;
    do {
        pipe_rv = detail::call_pipe2(ready_pipe, O_CLOEXEC);
    } while (pipe_rv == -1 && errno == EINTR);
    if (pipe_rv == -1) {
        return false;
    }

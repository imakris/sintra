...
struct Ring_R : Ring<T, true>
{
    ...
    const Range<T> wait_for_new_data()
    {
        ...
        auto deliver_pending_range = [&]() {
            ...
            const auto read_sequence = m_reading_sequence->load(std::memory_order_acquire);
            auto num_range_elements =
                size_t(c.leading_sequence.load(std::memory_order_acquire) - read_sequence);
            ...
            ret.begin = this->m_data + mod_u64(read_sequence, this->m_num_elements);

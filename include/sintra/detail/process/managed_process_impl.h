    ...
    detail::Release_mode release_mode)
{
    ...
    auto cleanup_worker = [this, custody, release_attempt_generation,
        ...
        // Exact historical convergence never performs process-slot operations.
        // Communication uses retained per-occurrence authority; native cleanup
        // uses the exact PID/start-stamp or Windows handle already in custody.
                ...
                detail::Release_mode release_mode) -> bool
        {
            for (const auto& [occurrence_number, process_iid] : targets) {
                ...
                const detail::Managed_child_occurrence_token token{
                    custody, process_iid, occurrence_number};
                bool native_terminal = true;
    ...
    const std::shared_ptr<Process_message_reader>& reader)
{
    ...
    std::shared_ptr<Process_message_reader> retired_authority;
    {
        ...
        auto exact = std::find_if(
            custody->occurrences.begin(), custody->occurrences.end(),
            [&](const detail::Managed_child_occurrence_record& occurrence) {
                return occurrence.process_instance_id == token.process_instance_id &&
                    occurrence.occurrence == token.occurrence;
            });
    ...
    const Spawn_swarm_process_args& s)
{
    ...
    const detail::Managed_child_occurrence_token occurrence_token{
        ...
        s.piid,
        s.occurrence};
    Instantiator initialization_reservation_guard(std::function<void()>([&]() {
        if (!initialization_reservation_acquired ||
            initialization_reservation_transferred)
        {
            return;
        }
    ...
    const bool exact_occurrence_lookup_failed =
        detail::managed_child_invariant_for_test(
            ...
            s.piid,
            s.occurrence);
    bool exact_occurrence_found = false;
    {
        std::lock_guard<std::mutex> lock(custody->mutex);
        const auto exact = std::find_if(
            custody->occurrences.begin(), custody->occurrences.end(),
        ...
        exact_occurrence_found = exact != custody->occurrences.end();
        if (!exact_occurrence_lookup_failed && exact_occurrence_found) {
            exact->initialization_reservation_active = true;
            initialization_reservation_acquired = true;
            custody->changed.notify_all();
        }
    }
    if (exact_occurrence_lookup_failed || !exact_occurrence_found) {
    ...
    if (spawn_ready) {
...

        {
            ...
            if (result.os_process_created && result.os_pid > 0) {
                ...
                {
                    occurrence.os_process_start_stamp = *stamp;
                    occurrence.os_process_start_stamp_available = true;
                }
            }
#ifdef _WIN32
            occurrence.os_process_handle = result.os_process_handle;
            occurrence.os_process_handle_owned = result.os_process_handle_owned;
    ...
    if (result.success) {
        ...
        if (result.os_process_created && spawned_process_handle) {
            ...
            start_child_custody_worker(
                [custody, occurrence_number, spawned_process_handle]() {
        ...
        if (spawned_process_handle) {
            {
                std::lock_guard<std::mutex> lock(custody->mutex);
                auto& occurrence = exact_occurrence_locked();
                if (!occurrence.os_process_created) {
                    occurrence.os_process_handle = 0;
                    occurrence.os_process_handle_owned = false;
                }
            }

...
{
    inline uint64_t monotonic_microseconds() noexcept
    {
        // Share the same cross-process monotonic timeline used by other IPC
        // primitives so timestamps remain comparable even when readers and
        // writers live in different processes.
        return detail::monotonic_now_us();
    }
} // namespace detail_ring
        ...
        auto produce_range = [&]() -> Range<T> {
            ...
            auto mark_guard_busy = [&]() -> bool {
                ...
                while (true) {
                    ...
                    const uint64_t now_us = detail_ring::monotonic_microseconds();

        ...
        if (new_trailing_octile != m_trailing_octile) {
            ...
            while (true) {
                if ((observed_token & Ring<T, true>::guard_token_present_mask) == 0) {
                    // Guard was cleared by the writer; leave the token untouched so the
                    // eviction path can reattach it.
                    break;
                }
                ...
                const uint8_t old_token = observed_token;
                if (guard_token.compare_exchange_strong(observed_token, desired_token,
                                                        std::memory_order_seq_cst,
                                                        std::memory_order_seq_cst)) {
                    ...
                    const uint8_t old_guarded_octile =
                        old_token & Ring<T, true>::guard_token_octile_mask;
                    c.read_access.fetch_sub(
                ...
                // CAS failed: observed_token now holds the new value; retry if guard remains.
            }
        }
    }

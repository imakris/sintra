...
struct Ring: Ring_data<T, READ_ONLY_DATA>
{
    ...
    {
        Reader_state_fields fields;
        uint32_t            word;

        constexpr Reader_state_union(uint32_t value = 0) : word(value) {}
    };

    static constexpr uint32_t make_reader_state(
        Reader_status status, uint8_t octile, bool guard_present)
    {
        Reader_state_union state{};
        state.fields.trailing_octile = static_cast<uint8_t>(octile & guard_token_octile_mask);
        state.fields.guard_token     = guard_present ? static_cast<uint8_t>(guard_token_present_mask |
                                                                        (octile & guard_token_octile_mask))
                                                     : 0;
        state.fields.status  = static_cast<uint8_t>(status);
        state.fields.padding = 0;
        return state.word;
    }

    static constexpr uint8_t reader_state_status(uint32_t state)
    {
        return Reader_state_union{state}.fields.status;
    }

    static constexpr uint32_t reader_state_with_status(uint32_t state, uint8_t status)
    {
        Reader_state_union updated{state};
        updated.fields.status = status;
        return updated.word;
    }

    static constexpr uint32_t reader_state_with_trailing(uint32_t state, uint8_t octile)
    {
        Reader_state_union updated{state};
        updated.fields.trailing_octile = static_cast<uint8_t>(octile & guard_token_octile_mask);
        return updated.word;
    }

    static constexpr bool reader_state_guard_present(uint32_t state)
    {
        return (Reader_state_union{state}.fields.guard_token & guard_token_present_mask) != 0;
    }

    static constexpr uint8_t reader_state_guard_octile(uint32_t state)
    {
        return static_cast<uint8_t>(Reader_state_union{state}.fields.guard_token &
                                    guard_token_octile_mask);
    }

    static constexpr uint32_t reader_state_apply_guard(uint32_t state, uint8_t guard_value)
    {
        Reader_state_union updated{state};
        updated.fields.guard_token = guard_value;
        if ((guard_value & guard_token_present_mask) != 0) {
            updated.fields.trailing_octile = static_cast<uint8_t>(guard_value & guard_token_octile_mask);
        }
        return updated.word;
    }
    ...
    template <typename F>
    static uint32_t reader_state_fetch_update(
        std::atomic<uint32_t>& state, std::memory_order order, F&& transform)
    {
        uint32_t current = state.load(reader_state_load_order(order));
        while (true) {
            uint32_t desired = transform(current);
    ...
    template <typename F>
    static uint32_t reader_state_fetch_update_if(std::atomic<uint32_t>& state,
                                                 ...
                                                 bool&                        updated)
    {
        uint32_t current = state.load(reader_state_load_order(order));
        while (true) {
            std::optional<uint32_t> desired = transform(current);
    ...
    struct cache_line_sized_t {
        struct Payload {
            ...
            struct Packed_reader_state {
                std::atomic<uint32_t> word{make_reader_state(READER_STATE_INACTIVE, 0, false)};
                ...
                struct Status_accessor {
                    ...
                    {
                        return Ring::reader_state_status(word->load(order));
                    }

                    void store(uint8_t value, std::memory_order order = std::memory_order_seq_cst) const
                    {
                        Ring::reader_state_update(*word, order, [value](uint32_t current) {
                ...
                struct Trailing_accessor {
                    ...
                    uint8_t load(std::memory_order order = std::memory_order_seq_cst) const
                    {
                        return Reader_state_union{word->load(order)}.fields.trailing_octile;
                    }
                ...
                struct Guard_accessor {
                    ...
                    uint8_t load(std::memory_order order = std::memory_order_seq_cst) const
                    {
                        return static_cast<uint8_t>(Reader_state_union{word->load(order)}.fields.guard_token &
                    ...
                    {
                        const uint32_t previous = Ring::reader_state_fetch_update(
                            *word, order, [value](uint32_t current) {
                                return Ring::reader_state_apply_guard(current, value);
                            });
                        return Reader_state_union{previous}.fields.guard_token;
                    }
                    ...
                    {
                        uint32_t state_snapshot = word->load(Ring::reader_state_load_order(failure));
                        ...
                        while (true) {
                            const uint8_t snapshot_bits = static_cast<uint8_t>(
                                Reader_state_union{state_snapshot}.fields.guard_token &
                            ...
                            const uint32_t desired_state =
                                Ring::reader_state_apply_guard(state_snapshot, desired);
                    ...
                    template <typename F>
                    uint32_t fetch_update_if(F&& transform,
                                             bool& updated,
                                             std::memory_order order = std::memory_order_seq_cst) const
                    {
                        return Ring::reader_state_fetch_update_if(
                            *word, order, std::forward<F>(transform), updated);
                    }
                };

                Status_accessor status() { return Status_accessor{&word}; }
                Status_accessor status() const
                {
                    return Status_accessor{const_cast<std::atomic<uint32_t>*>(&word)};
                }

                Trailing_accessor trailing_octile() { return Trailing_accessor{&word}; }
                Trailing_accessor trailing_octile() const
                {
                    return Trailing_accessor{const_cast<std::atomic<uint32_t>*>(&word)};
                }

                Guard_accessor guard_token() { return Guard_accessor{&word}; }
                Guard_accessor guard_token() const
                {
                    return Guard_accessor{const_cast<std::atomic<uint32_t>*>(&word)};
                }
            } reader_state{};

            auto status() { return reader_state.status(); }
            auto status() const { return reader_state.status(); }

            auto trailing_octile() { return reader_state.trailing_octile(); }
            auto trailing_octile() const { return reader_state.trailing_octile(); }

            auto guard_token() { return reader_state.guard_token(); }
...
struct Ring_R : Ring<T, true>
{
    ...
    Range<T> start_reading(size_t num_trailing_elements)
    {
        ...
        while (true) {
            ...
            auto guard_token_accessor = c.reading_sequences[m_rs_index].data.guard_token();
            bool guard_attached = false;
            guard_token_accessor.fetch_update_if(
                [&](uint32_t current) -> std::optional<uint32_t> {
                    if (Ring<T, true>::reader_state_status(current) ==
                        Ring<T, true>::READER_STATE_EVICTED) {
                        return std::nullopt;
                    }

                    return Ring<T, true>::reader_state_apply_guard(
                        current,
            ...
            if (!guard_attached) {
                c.read_access.fetch_sub(guard_mask, std::memory_order_seq_cst);
            ...
            if (confirmed_trailing_octile == trailing_octile) {
                if (c.reading_sequences[m_rs_index].data.status().load(std::memory_order_seq_cst) ==
                    Ring<T, true>::READER_STATE_EVICTED)
                {
                    m_reading.store(false, std::memory_order_seq_cst);
                    m_reading_lock = false;
            ...
            const uint32_t guard_snapshot = guard_token_accessor.fetch_update_if(
                [&](uint32_t current) -> std::optional<uint32_t> {
                    if (Ring<T, true>::reader_state_status(current) ==
                        Ring<T, true>::READER_STATE_EVICTED) {
                        return std::nullopt;
                    }
                    return Ring<T, true>::reader_state_apply_guard(current, 0);
                },
                guard_cleared,
                std::memory_order_seq_cst);
    ...
    void done_reading()
    {
        ...
        if (m_reading.load(std::memory_order_seq_cst)) {
            auto guard_token_accessor = c.reading_sequences[m_rs_index].data.guard_token();
            bool   guard_cleared       = false;
            const uint32_t previous_state = guard_token_accessor.fetch_update_if(
                [&](uint32_t current) -> std::optional<uint32_t> {
                    if (Ring<T, true>::reader_state_status(current) ==
                        Ring<T, true>::READER_STATE_EVICTED) {
                        return std::nullopt;
    ...
    void done_reading_new_data()
    {
        ...
        if (new_trailing_octile != m_trailing_octile) {
            ...
            const uint8_t  desired_guard = static_cast<uint8_t>(
                Ring<T, true>::guard_token_present_mask | static_cast<uint8_t>(new_trailing_octile));
            ...
            while (true) {
                ...
                const uint32_t previous_state = guard_token_accessor.fetch_update_if(
                    [&](uint32_t current) -> std::optional<uint32_t> {
                        ...
                        return Ring<T, true>::reader_state_apply_guard(current, desired_guard);
                    },
                    guard_updated,
                    std::memory_order_seq_cst);
    ...
    void reattach_after_eviction()
    {
        ...
        auto          guard_accessor = c.reading_sequences[m_rs_index].data.guard_token();

        while (true) {
            c.read_access.fetch_add(mask, std::memory_order_seq_cst);

            bool guard_updated = false;
            const uint32_t previous_state = guard_accessor.fetch_update_if(
                [&](uint32_t current) -> std::optional<uint32_t> {
                    if (Ring<T, true>::reader_state_guard_present(current)) {
                        return std::nullopt;
                    }
                ...
                for (int i = 0; i < max_process_index; ++i) {
                        ...
                        == Ring<T, false>::READER_STATE_ACTIVE)
                    {
                        ...
                        if (reader_seq < eviction_threshold || blocking_current_octile) {
                            ...
                            const uint32_t previous_state = c.reading_sequences[i]
                                                               .data.guard_token()
                                                               .fetch_update_if(
                                                                   [&](uint32_t current)
                                                                       ...
                                                                       const uint32_t cleared =
                                                                           Ring<T, false>::reader_state_apply_guard(current, 0);
                                                                       return Ring<T, false>::reader_state_with_status(
                                                                           cleared,
                                                                           Ring<T, false>::READER_STATE_EVICTED);
                                                                   },
                                                                   guard_evicted,
                                                                   std::memory_order_seq_cst);

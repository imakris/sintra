...
// Maximum amount of time (in microseconds) the writer will wait for a guard
// to clear before forcefully clearing it to recover from crashed or stalled
// readers. Set to 200ms as a conservative safety net.
#ifndef SINTRA_READER_GUARD_TIMEOUT_US
#define SINTRA_READER_GUARD_TIMEOUT_US 200000u
#endif
...
struct Ring: Ring_data<T, READ_ONLY_DATA>
{
    ...
    struct cache_line_sized_t {
        struct Payload {
            ...
            std::atomic<uint64_t> guard_acquired_at{0};
            ...
            for (int i = 0; i < max_process_index; ++i) {
                ...
                if (dead) {
                            ...
                            expected, uint8_t{0}, std::memory_order_seq_cst))
                    {
                        ...
                        slot.guard_acquired_at.store(0, std::memory_order_seq_cst);
                    }
...
struct Ring_R : Ring<T, true>
{
    ...
    Range<T> start_reading(size_t num_trailing_elements)
    {
        ...
        while (true) {
            ...
            c.reading_sequences[m_rs_index].data.guard_acquired_at.store(
                monotonic_now_ns() / 1000, std::memory_order_seq_cst);
        ...
        if (m_reading.load(std::memory_order_seq_cst)) {
                    ...
                    expected, uint8_t{0}, std::memory_order_seq_cst))
            {
                c.read_access.fetch_sub(
                    uint64_t(1) << (8 * m_trailing_octile), std::memory_order_seq_cst);
                c.reading_sequences[m_rs_index].data.guard_acquired_at.store(0, std::memory_order_seq_cst);
            }
    ...
    {
        auto& slot = c.reading_sequences[m_rs_index].data;
        if (slot.has_guard.load(std::memory_order_seq_cst) != 0) {
            return false;
        }
...
public:
    ...
    {
#ifdef SINTRA_ENABLE_SLOW_READER_EVICTION
        if (!m_evicted_since_last_wait.load(std::memory_order_seq_cst)) {
            auto& slot = c.reading_sequences[m_rs_index].data;
            if (slot.status.load(std::memory_order_seq_cst) == Ring<T, false>::READER_STATE_EVICTED) {
                handle_eviction_if_needed();
            }
            ...
            auto run_eviction_pass = [&]() {
                ...
                for (int i = 0; i < max_process_index; ++i) {
                        ...
                        == Ring<T, false>::READER_STATE_ACTIVE)
                    {
                        ...
                        uint8_t guard_flag =
                            c.reading_sequences[i].data.has_guard.load(std::memory_order_seq_cst);
...
#endif
#endif
            // Track when we enter the guard-wait loop for timeout-based recovery
            const uint64_t loop_start_us = monotonic_now_ns() / 1000;
            size_t iteration = 0;

            while (c.read_access.load(std::memory_order_seq_cst) & range_mask) {
                bool has_blocking_reader = false;

                // Check for stale guards every 1000 iterations to limit syscall overhead
                const bool check_timeout = (++iteration % 1000) == 0;
                const uint64_t now_us = check_timeout ? (monotonic_now_ns() / 1000) : 0;
                const uint64_t elapsed_us = check_timeout ? (now_us - loop_start_us) : 0;
                ...
                for (int i = 0; i < max_process_index; ++i) {
                    ...
                    if (guard != 0) {
                        ...
                        // We've been stuck waiting for this octile for longer than the timeout
                        if (check_timeout && elapsed_us >= SINTRA_READER_GUARD_TIMEOUT_US) {
                            uint64_t acquired_at = c.reading_sequences[i].data.guard_acquired_at.load(std::memory_order_seq_cst);
                            bool should_clear = false;

                            if (acquired_at == 0) {
                                // Guard without timestamp - treat as stale
                                should_clear = true;
                            } else if (now_us >= acquired_at && (now_us - acquired_at) >= SINTRA_READER_GUARD_TIMEOUT_US) {
                                // Guard with old timestamp
                                should_clear = true;
                            }

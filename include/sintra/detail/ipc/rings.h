    ...
    struct cache_line_sized_t {
        struct Payload {
            ...
            auto fetch_update_guard_token_if(F&& transform, bool& updated) {
                return reader_state.fetch_update_guard_token_if(std::forward<F>(transform), updated);
            }
...
struct Ring_R : Ring<T, true>
{
    ...
    const Range<T> wait_for_new_data()
    {
        ...
        // Outer loop: retry waiting after heartbeat timeouts
        while (true) {
            if (m_stopping) {
                return Range<T>{};
            }
                ...
                if (sleepy_index >= 0) {
                    while (true) {
                        ...
                        if (wait_status == sintra_ring_semaphore::wait_result::timeout) {
                            ...
                            // Timeout is a heartbeat - break to outer loop to retry phases
                            break;
                        }
            ...
            // Otherwise, loop back to retry all phases (heartbeat timeout with no data)
        }
    }

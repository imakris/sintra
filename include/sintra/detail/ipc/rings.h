...
struct Ring_R : Ring<T, true>
{
    ...
    bool handle_eviction_if_needed()
    {
        ...
        if (release_owner == Ring<T, true>::guard_release_writer) {
            // Eviction in progress.  Leave the guard cleared so the next iteration observes the
            // READER_STATE_EVICTED hand-off and performs the full recovery without leaking guards.
            return false;
        }

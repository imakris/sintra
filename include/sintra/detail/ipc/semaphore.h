...
static inline void sem_trace_post_error(
    const void*    addr,
    uint32_t       count,
    uint32_t       n,
    int            err) noexcept
...
{
    auto& st = W(b);
    if (st.init_flag.load(std::memory_order_acquire) == 2) {
        return;
    }
    long expected = 0;
    if (st.init_flag.compare_exchange_strong(expected, 1, std::memory_order_acq_rel, std::memory_order_acquire)) {
        ...
        st.init_flag.store(2, std::memory_order_release);
    }
    else {
...
static inline int umtx_wait_uint(
    uint32_t*                  addr,
    uint32_t                   expected,
    const struct _umtx_time*   timeout)
{
...
public:
    ...
    friend void test_get_win_semaphore_info(
        const interprocess_semaphore&,
        std::uint64_t&,
        std::wstring&);
#endif

    ...
    const Spawn_swarm_process_args& s )
{
...

    {
        ...
        const bool has_all_group = s_coord->m_groups.count("_sintra_all_processes") != 0;
        const bool has_external_group = s_coord->m_groups.count("_sintra_external_processes") != 0;
        if (!has_all_group || !has_external_group) {
            return false;
        }
    }

...
{
    // Keep sampling targets until we observe a quiescent snapshot. Messages may
    // arrive while we are waiting on previously captured targets; looping makes
    // the processing fence robust against such in-flight deliveries.
    while (true) {
        ...
        bool more_pending = false;
        {
            ...
            for (auto& [process_id, reader_ptr] : m_readers) {
                ...
                const auto req_target = reader.get_request_leading_sequence();
                if (reader.prepare_delivery_target(
                        Process_message_reader::Delivery_stream::Request,
                        req_target).wait_needed) {
                    more_pending = true;
                    break;
                }
        ...
        // New work appeared while we were waiting; loop again to include the
        // freshly published targets.
    }
}

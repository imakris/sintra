...
* **Rendezvous barriers** (`barrier<sintra::rendezvous_t>(name)`) simply ensure that every participant has reached the synchronization point. Messages published before the barrier might still be in flight or waiting to be handled, so this mode is appropriate when only aligned phase progression is needed-for example, coordinating the simultaneous start of a workload whose logic does not depend on the effects of earlier messages.
  **Warning:** rendezvous does not guarantee delivery or handling of messages sent before the barrier. Two peers can both reach the rendezvous while still missing each other's prior messages (A sends x, B sends y, both call rendezvous, neither is guaranteed to have received the other). Use it only when correctness does not depend on pre-barrier message delivery/handling, or when another synchronization mechanism already enforces that. Prefer delivery or processing fences when your correctness depends on pre-barrier messages being observed.

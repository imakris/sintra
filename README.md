...
3. Enable Run-Time Type Information (RTTI) in your build (`-frtti` for GCC/Clang or `/GR` for MSVC); Sintra relies on `typeid` for type resolution.
4. Sintra vendors the required Boost headers under `third_party/boost`,
   including `interprocess` and `fusion`, so no additional dependency setup is needed when using the repository as-is.
5. Explore the `example/` directory to see how to set up signal buses, channels, and
